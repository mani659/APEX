...
def main():
    print("=========================================================")
    print("RC002 Study 006: Entropy Reduction Through Volatility State")
    print("=========================================================")
    ...
    for symbol in symbols:
        ...
        for i, r in enumerate(dataset.records):
            ...
            vol_state = r.features.get("volatility_state", 0.0)
        ...
        exp_config = ExperimentConfig(
            experiment_name=f"Behavioral_Volatility_State_{symbol}",
    ...
    # 2. Conditioned Entropy (Compression Subset)
    df_comp = df_res[df_res['vol_state'] == -1.0]
    comp_counts = df_comp['response'].value_counts()
    comp_counts_arr = [comp_counts.get(cat, 0) for cat in categories]
    comp_entropy = calculate_entropy(comp_counts_arr)
    
    # 3. Conditioned Entropy (Expansion Subset)
    df_exp = df_res[df_res['vol_state'] == 1.0]
    exp_counts = df_exp['response'].value_counts()
    exp_counts_arr = [exp_counts.get(cat, 0) for cat in categories]
    exp_entropy = calculate_entropy(exp_counts_arr)
    
    # Which regime produced the lowest entropy?
    if comp_entropy < exp_entropy and len(df_comp) > 0:
        best_entropy = comp_entropy
        best_regime = "Compression"
        best_df = df_comp
        best_arr = comp_counts_arr
    elif len(df_exp) > 0:
        best_entropy = exp_entropy
        best_regime = "Expansion"
        best_df = df_exp
        best_arr = exp_counts_arr
    else:
    ...
    report_content = f"""# RC002 Study 006: Entropy Reduction Through Volatility State

## Final Research Conclusion
**{conclusion}**

### Target Hypothesis
Does the pre-event Volatility State reduce Behavioral Response entropy?
...
| **Conditioned Shannon Entropy ({best_regime})** | {best_entropy:.4f} | Represents uncertainty when conditioned on the optimal volatility state. |
...
- **Compression Entropy**: {comp_entropy:.4f} (N={len(df_comp)})
- **Expansion Entropy**: {exp_entropy:.4f} (N={len(df_exp)})
...
"""
    for sym in symbols:
        ...
        df_s_cond = df_s[df_s['vol_state'] == (-1.0 if best_regime == "Compression" else 1.0)]
...
- **Purity Check**: A reduction of {rel_reduction:.2f}% implies the Volatility State {"successfully" if conclusion != "NOT SUPPORTED" else "failed to"} polarize the structural outcome of a Behavioral Event.
- **Behavioral Mechanics**: {"The data shows clear structural fragmentation. Volatility states dictate behavior." if conclusion != "NOT SUPPORTED" else "The data indicates that volatility state has very little bearing on how an exhaustion bar ultimately behaves."}

### Verdict
The hypothesis that Volatility State systematically reduces the entropy of the Behavioral Response Taxonomy is **{conclusion}**. 
"""

    with open(os.path.join(output_dir, "Study_006_Report.md"), "w", encoding='utf-8') as f:
