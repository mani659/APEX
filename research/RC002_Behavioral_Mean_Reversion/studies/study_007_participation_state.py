...
def main():
    print("=========================================================")
    print("RC002 Study 007: Entropy Reduction Through Participation State")
    print("=========================================================")
    ...
    for symbol in symbols:
        ...
        exp_config = ExperimentConfig(
            experiment_name=f"Behavioral_Participation_State_{symbol}",
    ...
    # 2. Conditioned Entropy (Low Participation Subset)
    df_low = df_res[df_res['part_state'] == -1.0]
    low_counts = df_low['response'].value_counts()
    low_counts_arr = [low_counts.get(cat, 0) for cat in categories]
    low_entropy = calculate_entropy(low_counts_arr)
    
    # 3. Conditioned Entropy (High Participation Subset)
    df_high = df_res[df_res['part_state'] == 1.0]
    high_counts = df_high['response'].value_counts()
    high_counts_arr = [high_counts.get(cat, 0) for cat in categories]
    high_entropy = calculate_entropy(high_counts_arr)
    
    # Which regime produced the lowest entropy?
    if low_entropy < high_entropy and len(df_low) > 0:
        best_entropy = low_entropy
        best_regime = "Low Participation"
        best_df = df_low
        best_arr = low_counts_arr
    elif len(df_high) > 0:
        best_entropy = high_entropy
        best_regime = "High Participation"
        best_df = df_high
        best_arr = high_counts_arr
    else:
    ...
    report_content = f"""# RC002 Study 007: Entropy Reduction Through Participation State

## Final Research Conclusion
**{conclusion}**

### Target Hypothesis
Does the pre-event Participation State reduce Behavioral Response entropy?
...
| **Conditioned Shannon Entropy ({best_regime})** | {best_entropy:.4f} | Represents uncertainty when conditioned on the optimal participation state. |
...
- **Low Participation Entropy**: {low_entropy:.4f} (N={len(df_low)})
- **High Participation Entropy**: {high_entropy:.4f} (N={len(df_high)})
...
"""
    for sym in symbols:
        ...
        df_s_cond = df_s[df_s['part_state'] == (-1.0 if best_regime == "Low Participation" else 1.0)]
...
- **Purity Check**: A reduction of {rel_reduction:.2f}% implies the Participation State {"successfully" if conclusion != "NOT SUPPORTED" else "failed to"} polarize the structural outcome of a Behavioral Event.
- **Behavioral Mechanics**: {"The data shows clear structural fragmentation based on participation depth." if conclusion != "NOT SUPPORTED" else "The data indicates that tick/volume density has virtually no bearing on how an exhaustion bar ultimately behaves."}

### Verdict
The hypothesis that Participation State systematically reduces the entropy of the Behavioral Response Taxonomy is **{conclusion}**. 
"""

    with open(os.path.join(output_dir, "Study_007_Report.md"), "w", encoding='utf-8') as f:
