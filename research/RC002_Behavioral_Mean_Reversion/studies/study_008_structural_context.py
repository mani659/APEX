...
def main():
    print("=========================================================")
    print("RC002 Study 008: Entropy Reduction Through Structural Context")
    print("=========================================================")
    ...
    for symbol in symbols:
        ...
        exp_config = ExperimentConfig(
            experiment_name=f"Behavioral_Structural_Context_{symbol}",
    ...
    # 2. Conditioned Entropy (Structural Extreme Subset)
    df_extreme = df_res[df_res['struc_state'] == 1.0]
    extreme_counts = df_extreme['response'].value_counts()
    extreme_counts_arr = [extreme_counts.get(cat, 0) for cat in categories]
    extreme_entropy = calculate_entropy(extreme_counts_arr)
    
    # 3. Conditioned Entropy (Structural Neutral Subset)
    df_neutral = df_res[df_res['struc_state'] == -1.0]
    neutral_counts = df_neutral['response'].value_counts()
    neutral_counts_arr = [neutral_counts.get(cat, 0) for cat in categories]
    neutral_entropy = calculate_entropy(neutral_counts_arr)
    
    # Which regime produced the lowest entropy?
    if extreme_entropy < neutral_entropy and len(df_extreme) > 0:
        best_entropy = extreme_entropy
        best_regime = "Structural Extreme"
        best_df = df_extreme
        best_arr = extreme_counts_arr
    elif len(df_neutral) > 0:
        best_entropy = neutral_entropy
        best_regime = "Structural Neutral"
        best_df = df_neutral
        best_arr = neutral_counts_arr
    else:
    ...
    report_content = f"""# RC002 Study 008: Entropy Reduction Through Structural Context

## Final Research Conclusion
**{conclusion}**

### Target Hypothesis
Does the pre-event Structural Context reduce Behavioral Response entropy?
...
| :--- | :--- | :--- |
| **Baseline Shannon Entropy** | {base_entropy:.4f} | Represents the unconditioned uncertainty of the taxonomy. |
| **Conditioned Shannon Entropy ({best_regime})** | {best_entropy:.4f} | Represents uncertainty when conditioned on the optimal structural state. |
...
How did the transition matrix polarize when constrained to the {best_regime} state?
...
"""
    ...
    report_content += f"""
## 3. Alternative Regime Data

For transparency, here is the entropy generated during the opposite regime.
- **Structural Extreme Entropy**: {extreme_entropy:.4f} (N={len(df_extreme)})
- **Structural Neutral Entropy**: {neutral_entropy:.4f} (N={len(df_neutral)})

## 4. Cross-Market Validation ({best_regime} subset)

Does the entropy reduction hold true consistently across markets when constrained to {best_regime}?
...
"""
    for sym in symbols:
        ...
        df_s_cond = df_s[df_s['struc_state'] == (1.0 if best_regime == "Structural Extreme" else -1.0)]
...
- **Purity Check**: A reduction of {rel_reduction:.2f}% implies the Structural Context {"successfully" if conclusion != "NOT SUPPORTED" else "failed to"} polarize the structural outcome of a Behavioral Event.
- **Behavioral Mechanics**: {"The data shows clear structural fragmentation based on where the event occurs in the local range." if conclusion != "NOT SUPPORTED" else "The data indicates that structural position has virtually no bearing on how an exhaustion bar ultimately behaves."}

### Verdict
The hypothesis that Structural Context systematically reduces the entropy of the Behavioral Response Taxonomy is **{conclusion}**. 
"""

    with open(os.path.join(output_dir, "Study_008_Report.md"), "w", encoding='utf-8') as f:
